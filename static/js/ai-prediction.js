...
class AIPredictionManager {
    ...
    buildPrompt(match) {
        ...
        const match_time = match.match_time || match.match_date || '未知时间';
        ...
        // 计算不让球赔率隐含概率
        const ho = parseFloat(home_odds) || 2.0;
        const do_val = parseFloat(draw_odds) || 3.2;
        const ao = parseFloat(away_odds) || 2.8;
        const sum_inv = (1 / ho) + (1 / do_val) + (1 / ao);
        const home_prob = ((1 / ho) / sum_inv * 100).toFixed(1);
        const draw_prob = ((1 / do_val) / sum_inv * 100).toFixed(1);
        const away_prob = ((1 / ao) / sum_inv * 100).toFixed(1);
        const prob_str = `主胜 ${home_prob}% | 平局 ${draw_prob}% | 客胜 ${away_prob}%`;

        return `你是一位专业足球分析师。请对以下足球比赛进行深度分析预测：
...
联赛：${league_name}
比赛时间：${match_time}
当前赔率：主胜 ${home_odds} | 平局 ${draw_odds} | 客胜 ${away_odds}
赔率隐含概率：${prob_str}
