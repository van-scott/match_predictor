document.addEventListener('DOMContentLoaded', function() {
    ...
    if (mainContent && !document.getElementById('cl-entry')) {
        const clEntry = document.createElement('div');
        clEntry.id = 'cl-entry';
        clEntry.className = 'feature-card';
        clEntry.innerHTML = `
            ...
            <h3>欧冠模拟</h3>
            <p>模拟欧冠比赛，预测冠军归属</p>
            <button id="go-to-cl-btn" class="btn primary-btn">开始模拟</button>
        `;
        
        // 找到合适的位置插入卡片
        const firstSection = mainContent.querySelector('section');
        if (firstSection) {
            mainContent.insertBefore(clEntry, firstSection.nextSibling);
        } else {
            mainContent.appendChild(clEntry);
        }
        
        // 绑定点击事件
        document.getElementById('go-to-cl-btn').addEventListener('click', showChampionsLeagueSection);
    }
