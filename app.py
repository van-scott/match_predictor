    ...
    try:
        ...
        for m in matches:
            ...
            try:
                ...
                prob_str = f"主胜 {home_prob:.1f}% | 平局 {draw_prob:.1f}% | 客胜 {away_prob:.1f}%"
            except Exception:
                ho_f, do_f, ao_f = 2.0, 3.2, 2.8
                prob_str = "主胜 40.7% | 平局 35.7% | 客胜 36.5%"

            prompt = f"""你是一位专业足球分析师。请对以下足球比赛进行深度分析预测：
...
1.  赔率隐含概率分析：
    *   [根据隐含概率分析市场预期和倾向]
2.  近期状态：
    *   {home}：[分析主队近期战绩、胜率、攻防得失球等表现]
    *   {away}：[分析客队近期战绩、胜率、攻防得失球等表现]
3.  历史交锋：
    *   [解读双方历史对战结果及心理优势]
4.  主客场表现：
    *   {home}（主场）：[详细点评主队主场优势和战术特征]
    *   {away}（客场）：[详细点评客队客场表现与防守抗压能力]
...
*   置信度： [高 / 中 / 低]
*   核心理由：
    1.  [第一条核心理由，引用数据如进球率、历史战绩支持]
    2.  [第二条核心理由，引用数据如主客场胜率或模型概率支持]
    3.  [第三条核心理由，引用具体数据支持]
...
*   {home}的主场爆发力：[识别并分析主队潜在的主场抢分和定位球突袭风险]
*   {away}的客场进攻效率：[识别并分析客队防守反击与客场终结效率的潜在变数]
...
def _setup_scheduler():
    ...
    try:
        ...
        app.logger.warning("⚠️ APScheduler 未安装，定时任务不可用。请运行: pip install APScheduler")
    except Exception as e:
...
if __name__ == '__main__':
    ...
    app.logger.info("   每 10 分钟自动检查比赛结果")
